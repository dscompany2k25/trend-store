...
function detectHeadless(): boolean {
  ...
  // Only use positive headless signals — window.chrome absence is too broad (WebViews and some
  // Android builds legitimately lack it). Real automation is caught by webdriver/automationProps.
  const noPlugins = navigator.plugins && navigator.plugins.length === 0 && !isMobileUA;
  return noPlugins || /headlesschrome|electron\//.test(ua);
}
